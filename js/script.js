...
pizzaJson.map((item, index) => { //Fill content div with pizzas info & open modal system.
    ...
    pizzaItem.querySelector('.pizzaPrice').innerHTML = `R$ ${item.price.toFixed(2)}`;
...
/*     document.querySelector('') - Do span do header do mobile*/
    ...
    if (cart.length > 0) {
        ...
        document.querySelector('.cart .finalData .text .subTotal').innerHTML = `Subtotal: R$ ${subtotal.toFixed(2)}`;
        document.querySelector('.cart .finalData .text .discount').innerHTML = `Desconto: R$ ${discount.toFixed(2)}`
        document.querySelector('.cart .finalData .text .totalText').innerHTML = `Total: R$ ${total.toFixed(2)}`
...
finaliza.addEventListener('click', () => iniciaDados('modal-container-dados'));
...
function finalizarcomprar(){
  if (nome.value != '' && email.value != '') {
    ...
    msgSuccess.innerHTML = '<strong>Prontinho! Seu Pedido foi finalizado com sucesso! </strong>'
    msgError.setAttribute('style', 'display: none')
    msgError.innerHTML = ''
 } else {
  msgError.setAttribute('style', 'display: block')
  msgError.innerHTML = '<strong>Ops! Algo saiu errado. Precisamos de todos os dados Completos! </strong>'
